...
const HostLoginPage: FC = () => {
  const router = useRouter();
  const userType: UserType = 'hostAdmin';
  ...
  const handleLogin = async (data: { contactNumber: string; password: string }) => {
    try {
      // Do not add 852 prefix for hostAdmin, only for others
      const contactNumber = userType === 'hostAdmin'
        ? data.contactNumber
        : (data.contactNumber.startsWith('852') ? data.contactNumber : `852${data.contactNumber}`);
