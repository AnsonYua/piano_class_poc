...
const PageDashboard: FC<PageDashboardProps> = ({}) => {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("shop_owner_auth_token");
    if (!token) {
      router.push("/shop-owner-admin/login");
    }
  }, [router]);
  ...
  return (
    <div className="container mb-24 lg:mb-32">
      ...
      <div className="max-w-4xl mx-auto p-6 bg-white dark:bg-neutral-800 rounded-lg shadow-md">
        <p className="text-lg mb-4">歡迎來到琴行管理面板</p>
        <p className="mb-4">在這裡您可以管理您的琴行、課程和預約。</p>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
          <div className="p-4 bg-primary-50 dark:bg-neutral-700 rounded-lg">
            <h3 className="text-xl font-semibold mb-2">琴行管理</h3>
            <p>查看和管理您的琴行資料</p>
          </div>
