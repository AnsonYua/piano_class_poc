...
const PageLogin: FC<PageLoginProps> = ({}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/auth/shop-owner/login', {
        method: 'POST',
        headers: {
      ...
      // Store the token
      localStorage.setItem("shop_owner_auth_token", data.token);
        ...
        <div className="max-w-md mx-auto space-y-6">
          ...
          */}
          ...
          <form className="grid grid-cols-1 gap-6" onSubmit={handleSubmit} autoComplete="off">
            ...
            <label className="block">
              <span className="flex justify-between items-center text-neutral-800 dark:text-neutral-200">
                密碼
                <Link href={"/shop-owner-admin/forgot-password" as Route} className="text-sm underline font-medium">
                  忘記密碼
                </Link>
              </span>
          ...
          <span className="block text-center text-neutral-700 dark:text-neutral-300">
            沒有帳號? {` `}
            <Link href={"/shop-owner-admin/signup" as Route} className="font-semibold underline">
            創建帳戶
            </Link>
          </span>
        </div>
